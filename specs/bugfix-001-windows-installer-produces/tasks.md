...
- [X] T001 Inspect current release asset download flow in `install.ps1`
- [X] T002 [P] Verify expected Windows asset names in `.github/workflows/agent-release.yml`
- [X] T003 Create PowerShell regression test scaffold in `tests/windows-installer-assets.ps1`
...
- [X] T004 Add representative latest-release metadata fixtures in `tests/windows-installer-assets.ps1`
- [X] T005 Extract release metadata retrieval into a focused helper in `install.ps1`
- [X] T006 Extract exact-name asset URL resolution into a focused helper in `install.ps1`
- [X] T007 Update `tests/windows-installer-assets.ps1` to dot-source or load only the helper behavior from `install.ps1`
...
- [X] T008 [P] [US1] Add regression test for resolving agent asset `browser_download_url` in `tests/windows-installer-assets.ps1`
- [X] T009 [P] [US1] Add regression test for resolving capture asset `browser_download_url` in `tests/windows-installer-assets.ps1`
...
- [X] T011 [US1] Replace manual tag-based agent download URL construction with release metadata URL resolution in `install.ps1`
- [X] T012 [US1] Replace manual tag-based capture download URL construction with release metadata URL resolution in `install.ps1`
- [X] T013 [US1] Update installer status output to report the selected release tag from metadata in `install.ps1`
- [X] T014 [US1] Run `tests/windows-installer-assets.ps1` and confirm US1 tests pass
...
- [X] T015 [P] [US2] Add regression test for missing agent asset failure in `tests/windows-installer-assets.ps1`
- [X] T016 [P] [US2] Add regression test for missing capture asset failure in `tests/windows-installer-assets.ps1`
...
- [X] T018 [US2] Add required-asset missing error handling before download invocation in `install.ps1`
- [X] T019 [US2] Ensure missing-asset errors include the asset name and release tag in `install.ps1`
- [X] T020 [US2] Ensure missing-asset failures cannot create or overwrite destination binaries in `install.ps1`
- [X] T021 [US2] Run `tests/windows-installer-assets.ps1` and confirm US2 tests pass
...
- [X] T022 [P] [US3] Add regression test that `-SkipTray` skips tray asset lookup in `tests/windows-installer-assets.ps1`
- [X] T023 [P] [US3] Add regression test that tray asset resolves through `browser_download_url` when tray is enabled in `tests/windows-installer-assets.ps1`
- [ ] T024 [US3] Run `tests/windows-installer-assets.ps1` and confirm the new US3 tests fail before updating tray handling

### Implementation for User Story 3

- [X] T025 [US3] Replace manual tray download URL construction with release metadata URL resolution when `-SkipTray` is not set in `install.ps1`
- [X] T026 [US3] Preserve `-SkipTray` behavior so tray lookup and download are skipped in `install.ps1`
- [X] T027 [US3] Review changed installer output for plain-text, copyable, non-color-dependent messages in `install.ps1`
- [X] T028 [US3] Confirm English-only output remains consistent with existing installer scope in `install.ps1`
- [X] T029 [US3] Run `tests/windows-installer-assets.ps1` and confirm US3 tests pass
...
- [X] T030 Run PowerShell parser validation for `install.ps1` using the command from `specs/bugfix-001-windows-installer-produces/quickstart.md`
- [X] T031 Update regression test status and test description in `specs/bugfix-001-windows-installer-produces/bug-report.md`
- [X] T032 Update verification checklist in `specs/bugfix-001-windows-installer-produces/bug-report.md`
- [X] T033 [P] Document parser validation and targeted regression results in `specs/bugfix-001-windows-installer-produces/quickstart.md`
- [X] T034 Run `just lint` and `just check` if files beyond `install.ps1`, `tests/windows-installer-assets.ps1`, and `specs/bugfix-001-windows-installer-produces/` changed
