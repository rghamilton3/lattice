	...
	new Elysia()
		.get(
			...
			async ({ query, set }) => {
				...
				switch (source.kind) {
					case 'capture': {
						const row = db.query('SELECT text FROM captures WHERE id = ?').get(source.id) as {
							text: string;
						} | null;
						if (!row) {
							set.status = 404;
					...
					case 'local-file': {
						const row = db.query('SELECT text FROM file_index WHERE id = ?').get(source.id) as {
							text: string;
						} | null;
						if (!row) {
							set.status = 404;
		...
		.get(
			...
			({ query, set }) => {
				...
				const lower = new Date(base.getTime() - windowHours * 3_600_000).toISOString();
				const upper = new Date(base.getTime() + windowHours * 3_600_000).toISOString();
