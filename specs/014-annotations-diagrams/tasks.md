...
- [ ] T001 Create SQLite annotations table migration with constraints for target kind, target id, optional offsets, selected text, non-empty comment, and timestamps in `spine/migrations/012_annotations.sql`
- [ ] T002 [P] Add shared Annotation, AnnotationTargetKind, AnnotationCreateInput, and AnnotationListResponse API types in `surface/src/lib/types.ts`
- [ ] T003 [P] Add spine-side annotation row/input/result types and supported target kind constants in `spine/src/annotations.ts`
...
- [ ] T004 Implement annotation input validation helpers for target kind, target id, offsets, selected text, and trimmed comment in `spine/src/annotations.ts`
- [ ] T005 [P] Create typed Surface annotation API wrapper for list, create, and delete requests in `surface/src/lib/api/annotations.ts`
- [ ] T006 [P] Extend selection utilities to return trimmed selected text plus best-effort character offsets relative to a reading container in `surface/src/lib/utils/selection.ts`
- [ ] T007 [P] Add annotation API query keys for target-scoped annotation lists and mutations in `surface/src/lib/api/annotations.ts`
- [ ] T008 Add annotation route import and authenticated browser route registration placeholder in `spine/src/app.ts`
...
- [ ] T010 [P] [US1] Add route tests for `POST /api/annotations`, `GET /api/annotations`, and `DELETE /api/annotations/:id` success responses in `spine/tests/routes/annotations.test.ts`
- [ ] T011 [P] [US1] Add route validation tests for empty comments, invalid target kinds, invalid offsets, empty selections, missing query parameters, and unknown ids in `spine/tests/routes/annotations.test.ts`
...
- [ ] T015 [US1] Implement annotation repository functions for create, list by target, delete by id, and row mapping in `spine/src/annotations.ts`
- [ ] T016 [US1] Implement `POST /api/annotations`, `GET /api/annotations`, and `DELETE /api/annotations/:id` Elysia routes using browser auth in `spine/src/routes/annotations.ts`
- [ ] T017 [US1] Wire `annotationsRoutes(db)` into the authenticated route group in `spine/src/app.ts`
- [ ] T018 [US1] Implement `fetchAnnotations`, `createAnnotation`, and `deleteAnnotation` functions using `apiFetch` in `surface/src/lib/api/annotations.ts`
- [ ] T019 [US1] Add target identity derivation for capture, local file, working doc, and archive reading states in `surface/src/components/reading/ReadingPane.svelte`
- [ ] T020 [US1] Load and cache target-scoped annotations with TanStack Query in `surface/src/components/reading/ReadingPane.svelte`
- [ ] T021 [US1] Add keyboard-operable annotation creation UI for selected passages, empty selection errors, empty comment errors, and non-urgent copy in `surface/src/components/reading/ReadingPane.svelte`
- [ ] T022 [US1] Render saved annotation highlights and associated notes without changing source content in `surface/src/components/reading/ReadingPane.svelte`
- [ ] T023 [US1] Add keyboard-operable delete controls with mutation invalidation and non-urgent confirmation/error copy in `surface/src/components/reading/ReadingPane.svelte`
- [ ] T024 [US1] Add non-color-only highlight styling, visible focus indicators, accessible names, and semantic note associations in `surface/src/components/reading/ReadingPane.svelte`
...
- [ ] T027 [P] [US2] Add search route tests proving annotation text returns the annotated source and deleted annotation text stops matching in `spine/tests/routes/search.test.ts`
...
- [ ] T029 [US2] Add annotation QMD directory helper, markdown serializer, write helper, and delete helper in `spine/src/search.ts`
- [ ] T030 [US2] Include existing annotations in `initSearch`, register the `annotations` QMD collection, and refresh the index after annotation file changes in `spine/src/search.ts`
- [ ] T031 [US2] Call annotation index write on create and annotation index delete on delete from `spine/src/annotations.ts`
- [ ] T032 [US2] Map QMD annotation collection hits back to original target kind, target id, annotation id, snippet, title fallback, and modified timestamp in `spine/src/search.ts`
- [ ] T033 [US2] Extend Surface `SearchResult` union with annotation-origin fields `target_kind`, `target_id`, and `annotation_id` in `surface/src/lib/types.ts`
- [ ] T034 [US2] Update search result rendering to expose annotation-match context and route to the annotated source instead of an annotation-only page in `surface/src/components/search/ResultRow.svelte`
- [ ] T035 [US2] Pass annotation reveal context from search results into reading pane navigation state in `surface/src/lib/state/workbench.svelte.ts`
- [ ] T036 [US2] Reveal or focus the matching annotation when opening an annotation-origin result in `surface/src/components/reading/ReadingPane.svelte`
...
- [ ] T041 [US3] Add keyboard-operable insert diagram command that inserts the fenced Mermaid template at the current cursor or selection in `surface/src/components/editor/EditorPane.svelte`
- [ ] T042 [US3] Add editor toolbar or command-palette entry for diagram insertion with accessible name, visible focus, and no pointer-only dependency in `surface/src/components/editor/EditorPane.svelte`
- [ ] T043 [US3] Ensure valid fenced `mermaid` blocks render inline through the existing Mermaid path without stripping source markdown in `surface/src/components/reading/MarkdownRenderer.svelte`
- [ ] T044 [US3] Add clear, screen-reader-understandable, keyboard-reachable non-destructive Mermaid render error UI in `surface/src/components/reading/MarkdownRenderer.svelte`
...
- [ ] T046 [P] Run spine test suite and fix failures from annotation schema, routes, and search indexing in `spine/`
- [ ] T047 [P] Run Surface test suite and fix failures from annotation UI, search result UI, and diagram insertion/rendering in `surface/`
...
- [ ] T050 Run repository lint and type checks, then fix all feature-related issues in `spine/`, `surface/`, and `specs/014-annotations-diagrams/tasks.md`
