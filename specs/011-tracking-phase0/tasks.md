...
- [ ] T001 Create tracking schema migration with `tracks` and `track_queries` tables plus indexes in `spine/migrations/010_tracks.sql`
- [ ] T002 [P] Add shared tracking row/request/response TypeScript interfaces in `spine/src/db/rows.ts`
- [ ] T003 [P] Create placeholder authenticated browser tracking route module in `spine/src/routes/tracks.ts`
- [ ] T004 Register `tracksRoutes(db)` inside the existing Authentik guard in `spine/src/app.ts`
...
- [ ] T005 Add migration/schema tests for `tracks`, `track_queries`, indexes, and foreign keys in `spine/src/tracks.schema.test.ts`
- [ ] T006 [P] Add test database setup helpers for route tests in `spine/src/testSupport/db.ts`
- [ ] T007 [P] Add app request/auth helper utilities for bearer-token and Authentik-style test requests in `spine/src/testSupport/http.ts`
- [ ] T008 Cover track request normalization and validation through route tests in `spine/src/routes/agent.track.test.ts`
- [ ] T009 Run `bun test spine/src/tracks.schema.test.ts` and fix schema/setup failures in `spine/migrations/010_tracks.sql`
...
- [ ] T010 [P] [US1] Add route tests for successful normal and displaced `POST /api/agent/track` writes in `spine/src/routes/agent.track.test.ts`
- [ ] T011 [P] [US1] Add route tests for blank text, missing source, invalid displaced values, invalid supersession, and missing bearer token in `spine/src/routes/agent.track.test.ts`
- [ ] T012 [P] [US1] Add append-only regression test proving duplicate/stale/friction text creates additional rows without editing prior rows in `spine/src/routes/agent.track.test.ts`

### Implementation for User Story 1

- [ ] T013 [US1] Implement `POST /track` inside existing `agentRoutes` body schema in `spine/src/routes/agent.ts`
- [ ] T014 [US1] Insert tracking rows with spine-set `ingested_at`, explicit boolean-to-integer `displaced`, nullable `photo_ref`, and nullable `supersedes` in `spine/src/routes/agent.ts`
- [ ] T015 [US1] Validate optional `supersedes` references an existing `tracks.id` before insert in `spine/src/routes/agent.ts`
- [ ] T016 [US1] Run `bun test spine/src/routes/agent.track.test.ts` and fix write-path failures in `spine/src/routes/agent.ts`
...
- [ ] T017 [P] [US2] Add route tests for `GET /api/tracks/search?q=drill` result shape and newest-first ordering in `spine/src/routes/tracks.test.ts`
- [ ] T018 [P] [US2] Add route tests for blank query rejection, missing Authentik auth rejection, and empty/no-match response behavior in `spine/src/routes/tracks.test.ts`
- [ ] T019 [P] [US2] Add route tests for `POST /api/tracks/queries/:id/open` success, unknown query, unknown track, and invalid `track_id` in `spine/src/routes/tracks.test.ts`

### Implementation for User Story 2

- [ ] T020 [US2] Implement keyword track search with query logging in `spine/src/routes/tracks.ts`
- [ ] T021 [US2] Return `query_id` plus result fields `id`, `text`, `captured_at`, `ingested_at`, `source`, `displaced`, `photo_ref`, and `supersedes` from `spine/src/routes/tracks.ts`
- [ ] T022 [US2] Implement result-open logging endpoint `POST /api/tracks/queries/:id/open` in `spine/src/routes/tracks.ts`
- [ ] T023 [US2] Run `bun test spine/src/routes/tracks.test.ts` and fix search/open failures in `spine/src/routes/tracks.ts`
...
- [ ] T024 [P] [US3] Add HA payload smoke-test examples for normal and checkout requests in `spine/src/routes/agent.track.test.ts`

### Implementation for User Story 3

- [ ] T025 [US3] Document HA Voice sentence triggers, payload fields, fallback phrases, and 5-second verification in `specs/011-tracking-phase0/quickstart.md`
- [ ] T026 [US3] Add a HA Voice manual verification checklist with pass/fail rows in `specs/011-tracking-phase0/ha-voice-verification.md`
- [ ] T027 [US3] Execute the HA Voice verification checklist and record results or blockers in `specs/011-tracking-phase0/ha-voice-verification.md`
...
- [ ] T028 [P] [US4] Add Signal parser tests for `/track`, `/checkout`, blank command content, and existing `/capture` preservation in `spine/src/signal/messages.test.ts`
- [ ] T029 [P] [US4] Add Signal relay posting tests for track versus capture endpoint selection and displaced state payloads in `spine/src/signal-relay.test.ts`

### Implementation for User Story 4

- [ ] T030 [US4] Extend parsed Signal message types to distinguish capture, track, checkout, and optional photo tracking payloads in `spine/src/signal/messages.ts`
- [ ] T031 [US4] Route Signal `/track` and `/checkout` messages to `/api/agent/track` while preserving existing capture routing in `spine/src/signal-relay.ts`
- [ ] T032 [US4] Preserve Signal photo caption text and optional stored photo reference for tracking payloads in `spine/src/signal-relay.ts`
- [ ] T033 [US4] Document Tasker phrase setup, Signal command setup, assistant-interception failure criteria, and 10-second verification in `specs/011-tracking-phase0/quickstart.md`
- [ ] T034 [US4] Add portable and Signal manual verification checklist with pass/fail rows in `specs/011-tracking-phase0/portable-signal-verification.md`
- [ ] T035 [US4] Run `bun test spine/src/signal/messages.test.ts spine/src/signal-relay.test.ts` and fix routing regressions in `spine/src/signal/messages.ts` and `spine/src/signal-relay.ts`
...
- [ ] T036 [P] Add quickstart curl verification transcript placeholders for normal track, checkout track, search, and result-open flows in `specs/011-tracking-phase0/quickstart.md`
- [ ] T037 [P] Add accessibility evidence or N/A rationale for API/device-only Phase 0 work in `specs/011-tracking-phase0/accessibility.md`
- [ ] T038 [P] Add bilingual delivery N/A rationale and any English-only user-facing copy review notes in `specs/011-tracking-phase0/accessibility.md`
- [ ] T039 [P] Add CLI/terminal output accessibility review for curl/manual setup errors in `specs/011-tracking-phase0/accessibility.md`
- [ ] T040 Run `just test` from repository root and fix failures in `spine/src/`
- [ ] T041 Run `just check` from repository root and fix TypeScript/Svelte check failures in `spine/src/` or `surface/src/`
- [ ] T042 Run the complete quickstart smoke test and record final results in `specs/011-tracking-phase0/quickstart.md`
