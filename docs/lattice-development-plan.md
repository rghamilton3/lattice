...
- **Inference layer** — Ollama serving QMD's three GGUF models (embedder, reranker, expansion) via an OpenAI-compatible API. Deployed on a dedicated home GPU server (RTX 4070 Ti Super 16GB on Ryzen 7 5800X3D), reachable from the VPS over Tailscale. Same host also runs other GPU-bound workloads (currently Obico ML for 3D-print failure detection; future Phase 9+ workloads). Spine talks to it through QMD's `RemoteLLM` path (adopted from upstream PR #629 + Kaspre's documented fixes). The `RemoteLLM` boundary is provider-agnostic, so future migration to a different host (cloud, replacement hardware, etc.) is a config change.
...
- **Auth** — Caddy + Authelia forward auth for the surface (browser sessions). Bearer-token bypass for `/api/agent/*` routes (programmatic). Spine → Ollama is over Tailscale, which provides the authentication boundary at the network layer; no application-level token currently. If the inference layer moves to a public endpoint in a future phase, add bearer-token auth at that boundary.
...
Deployment target for the inference layer was deliberated at length: RunPod serverless, RunPod pods, Vultr fractional A40, home consumer GPU, and a Jetson Orin Nano 8GB were all considered seriously. The decision pivoted twice. First, to the Orin Nano (zero marginal cost, hardware on hand, explicit "revisit when MSCS lands" framing). Then, once a free RTX 4070 Ti Super (16GB) became available, to a dedicated home GPU server built from previously-owned parts (Ryzen 7 5800X3D, RAM, PSU). The 4070 Ti Super is meaningfully more capable than the Orin and consolidates other GPU-bound personal workloads (Obico ML for 3D printing, future Whisper/vision models) onto a single host. The Orin Nano is freed for its intended UGV role; the previously-Obico-hosting Jetson Nano is freed entirely.

The "revisit when MSCS lands" framing is *mostly* resolved — the 4070 Ti Super covers expected coursework — but kept as a written future trigger for the specific case where program requirements demand datacenter-tier GPU access (multi-GPU training, very large models, etc.) that would route to cloud or campus resources rather than this box.
...
**Goal**: QMD's hot path runs in well under a second instead of 60+. The Spine offloads model inference to Ollama running on a dedicated home GPU server, reachable from the VPS over Tailscale. The same host consolidates other GPU-bound personal workloads (Obico ML for 3D printing now; Whisper/vision/etc. in Phase 9+). Establishes the OpenAI-compatible inference layer that future ML workloads plug into without re-architecting.
...
### Deployment target: home GPU server

The deployment-target deliberation went through several iterations (Orin Nano, Vultr fractional A40, RunPod) before landing here. The decisive change was a free RTX 4070 Ti Super (16GB) becoming available, alongside previously-owned parts (Ryzen 7 5800X3D, RAM, PSU). That hardware combination is a legitimate ML workstation at zero marginal cost.

The architectural pattern is **GPU services consolidation**: one home server hosts every workload in the household that benefits from GPU acceleration. Application-layer services that don't need a GPU (Spine, Surface, SQLite, Caddy, etc.) stay on the VPS where capture-anywhere reliability and public-IP reachability matter. The GPU server is reached from the VPS over Tailscale; never publicly exposed.

What lives on the GPU server:

- **Ollama** serving QMD's three models for Lattice (this phase)
- **Obico ML** for 3D-print failure detection, migrated off the existing Jetson Nano which was running it on the ragged edge of its capability (this phase, opportunistic)
- **Future workloads**: Whisper for voice transcription, vision models for OCR/image annotation, additional models as Phase 9+ needs them. The 16GB VRAM is plenty to keep all of these resident simultaneously.

What doesn't live on the GPU server: anything Lattice-specific that doesn't need a GPU. The Spine stays on the VPS. Backups stay on their existing off-VPS target.

Freed hardware: the Jetson Orin Nano 8GB goes back to its intended UGV role; the older Jetson Nano (4GB) is freed entirely.

**Why this is the right answer here:**

- **Zero marginal cost.** All hardware was already owned or freely gifted. Cloud paths would have added $60–200+/month indefinitely.
- **Significantly more capable than the alternatives.** The 4070 Ti Super outperforms Vultr's fractional A40 slices, beats the Orin Nano by ~10× for inference, and has 2× the VRAM of the largest Vultr slice that was on the table ($0.288/hr 8GB tier).
- **Consolidates other workloads.** Obico moves here as a bonus, removing the Jetson Nano as a maintenance burden. Phase 9+ workloads have a home already.
- **MSCS coverage.** A 4070 Ti Super handles most ML coursework comfortably. The "revisit when MSCS lands" trigger from earlier drafts is largely resolved by this hardware; what remains is the narrow case of coursework demanding datacenter-tier GPU access (multi-GPU training, very large models), which would route to cloud or campus resources rather than displacing this server.

**Why the home network is OK for this:**

The home internet/power are reliable enough that residential hosting works for inference. The graceful BM25 fallback in QMD's `HybridLLM` handles individual outages without breaking search entirely — searches just degrade to lexical-only when the GPU host is unreachable, and the surface displays an honest "deep search offline" indicator. The capture-anywhere path doesn't depend on the GPU host at all; captures land on the Spine (VPS), get indexed asynchronously, and are searchable via BM25 immediately.

**Future revisit trigger** (much narrower now than in earlier drafts): if MSCS coursework requires datacenter-tier GPU compute that the 4070 Ti Super can't provide, evaluate cloud GPU credits (most programs include some), or migrate specific workloads to cloud while keeping Lattice's inference layer here.
...
- **Ollama**: handles GGUF natively (which is what QMD's models ship as), serves multiple models from one instance, exposes an OpenAI-compatible API, well-documented, Docker-friendly. The right answer for this scale.
- **vLLM**: faster at high QPS via PagedAttention batching, but needs non-GGUF model versions, serves one model per instance, and the throughput advantage is wasted at one-user scale.
...
Default is Ollama. Revisit if a specific workload outgrows it (vLLM becomes interesting if a generative workload needs serious throughput at scale; a custom shim becomes interesting if a model isn't available in any standard server).

### Scope

**GPU server build and base setup** (one evening, low complexity):

- Hardware: RTX 4070 Ti Super, Ryzen 7 5800X3D, existing RAM, 1000W PSU (comfortable headroom; a more efficient PSU is a clean future drop-in), case. NVMe SSD for the boot drive.
- Detailed operational walkthrough: see `docs/gpu-server-setup.md`.
- OS: **Ubuntu Server 26.04 LTS** (24.04 LTS works identically if 26.04 isn't out yet). Marginally smoother NVIDIA setup (`ubuntu-drivers autoinstall`, signed driver works with Secure Boot, drivers in main repo), more public docs and reference container images target Ubuntu. The host OS is substrate for Ansible-driven management; the day-to-day commands are identical between Ubuntu and Debian, so picking the slightly smoother NVIDIA path is the right call. Headless install — no desktop environment. SSH-only access; revisit if a desktop becomes useful.
- SSH access, set up keys, disable password auth.
- Install Tailscale on both the server and the VPS. Network boundary is the tailnet.
- Install NVIDIA drivers via Ubuntu's `nvidia-driver-XXX` packages (or `ubuntu-drivers autoinstall`). Reboot. Confirm `nvidia-smi` shows the 4070 Ti Super and driver version.
- Install Docker (for Obico) and the NVIDIA Container Toolkit (so containerized workloads see the GPU).
- Configure auto-login disabled, unattended-upgrades enabled for security patches, fail2ban on SSH. Standard always-on-server hygiene.
- Set up off-server backups for any local state (Modelfiles, docker-compose configs, etc.) — same backup discipline as the VPS.

**Ollama deployment** (an hour, low complexity):

- Install Ollama via the official installer (`curl -fsSL https://ollama.com/install.sh | sh`). It detects NVIDIA and installs CUDA-enabled binaries.
- Edit the systemd unit (`sudo systemctl edit ollama`):
  - `Environment="OLLAMA_HOST=0.0.0.0:11434"` so Tailscale can reach it.
  - `Environment="OLLAMA_KEEP_ALIVE=24h"` so models stay resident between sparse queries.
  - `Environment="OLLAMA_MODELS=/var/lib/ollama/models"` pointing at fast NVMe storage.
- Pull/import the three QMD models:
  - `ollama pull embeddinggemma:300m` (direct from Ollama's registry).
  - `qwen3-reranker:0.6b` and `qmd-query-expansion:1.7b` — import via Modelfile pointing at HuggingFace-hosted GGUFs. Tobi's HF page has the exact recipe for the expansion model.
- Smoke test the OpenAI-compatible endpoint from the VPS via Tailscale before wiring into Spine.

**Obico ML migration** (an hour, opportunistic — do alongside Ollama or skip if you'd rather defer):

- Run Obico's ML server in Docker per their official documentation. Should run alongside Ollama without conflict — both can hold models in VRAM simultaneously with 16GB available.
- Update the Bambuddy / OctoPrint / Klipper integration to point at the new ML server URL.
- Decommission the Jetson Nano once the new server is confirmed working for a few prints.
...
- New env vars (in `spine/.env`):
  - `QMD_EMBED_API_URL=http://<gpu-server-tailscale-ip>:11434/v1`
  - `QMD_EMBED_API_MODEL=embeddinggemma:300m`
  - `QMD_RERANK_API_URL=http://<gpu-server-tailscale-ip>:11434/v1`
  - `QMD_RERANK_API_MODEL=qmd-rerank` (or whatever the imported tag is)
  - `QMD_EXPAND_API_URL=http://<gpu-server-tailscale-ip>:11434/v1`
  - `QMD_EXPAND_API_MODEL=qmd-expand`
  - `QMD_API_KEY=` left blank — Tailscale is the auth boundary
...
- Graceful degradation: if the GPU server is unreachable (power outage, Tailscale issue, reboot for updates, etc.), QMD's `HybridLLM` already falls back where it can (BM25 still works). The Surface displays a "search is in keyword-only mode (deep search offline)" indicator — necessary information, not a hidden failure.

**Workload coexistence considerations**:

- Obico runs **continuous inference** during 3D prints (not bursty like Lattice queries). Worst case is searching while printing — Lattice's hot path may go from sub-second to ~1–2s under that contention. Acceptable. Worth measuring once both are running to see if it's even noticeable in practice.
- VRAM doesn't contend: Obico's model (~1–2GB) + QMD's three models (~2.5GB) leaves ~10GB free. Plenty of headroom for Phase 9+ Whisper (~3GB) + vision model (~3–5GB) all simultaneously.
- `OLLAMA_KEEP_ALIVE=24h` is the important setting — without it, Ollama would unload models during idle periods and a print starting could trigger model evictions that hurt search latency.
...
- Track host metrics on the GPU server (Prometheus + node_exporter + nvidia_gpu_exporter is the standard, but for a one-user system a daily cron writing `nvidia-smi` + `uptime` + `df -h` to a log file is fine too). The metrics that matter: latency, fallback rate, GPU utilization, VRAM usage, server uptime.
- Set up a "weekly health summary" that emails or notifies you with: median/p95/p99 search latency, fallback rate, total search count, GPU server uptime, any concerning trends. Cron + a small script is enough.

### Deferred

- Buying additional GPU hardware. Becomes interesting only if MSCS coursework demands more than the 4070 Ti Super can provide.
- Moving inference to cloud. Becomes interesting only in narrow MSCS scenarios noted above.
- vLLM swap. Only worth the work if throughput becomes a measurable bottleneck — won't at one-user scale for years.
- Multi-region or multi-provider failover. The graceful BM25 fallback handles single-host outages adequately.
- Streaming responses. Not relevant for embed/rerank/expand; would matter if generative workloads land here.
- Splitting models across multiple Ollama instances. One Ollama serving multiple models is simplest.
- GPU passthrough to VMs, Proxmox, full homelab. Out of scope; this is a focused GPU services host, not a virtualization platform.

### Done when

- A full hybrid search via the Spine API returns in under a second end-to-end on warm models. Target: <500ms. Tolerable ceiling: <2s.
- The Spine is configured against the GPU server's Tailscale URL and serving real traffic.
- The fork of QMD with #629 + Kaspre's fixes is published somewhere we control and pinned in `spine/package.json`.
- Lexical-only fallback works — verified once by powering off the GPU server and confirming the surface returns BM25 results with the degraded indicator.
- Obico ML is running on the new server and has handled at least one full print successfully; Jetson Nano is offline / freed.
- A weekly health summary is being generated with latency, fallback rate, uptime, and resource trends.
...
- Wire existing clustering / resurfacing logic into the spine (sidecar process or scheduled job). Embedding-based clustering issues a batch of embedding requests through QMD → Ollama. Batch jobs run comfortably on the GPU server; not latency-sensitive.
...
After Phase 8, next moves depend entirely on what continued use reveals. The Ollama inference layer from Phase 6 is the deployment substrate for additional models; adding a new workload is "pull a model in Ollama and add env vars to the Spine," not "build a new infrastructure path." The 16GB VRAM headroom on the GPU server comfortably hosts Whisper, vision models, and additional generative work alongside the existing QMD models without juggling. Each candidate below is tagged with where it would run.
...
- **Voice capture transcription** *(Ollama on the GPU server, with Whisper)*. Add a Whisper GGUF to the same Ollama instance, expose via the same OpenAI-compatible audio endpoint, route from a new `/api/agent/transcribe` Spine route. Hosted alternative is Groq's Whisper which is fast and cheap — worth a bakeoff if volume gets meaningful.
- **Image annotation / OCR** *(Ollama on the GPU server, vision model)*. Pull a vision model into the same Ollama instance. Latency-tolerant. Hosted alternative is MathPix (specialized) or a hosted VLM.
...
- **Inference target migration** *(infra)* — if the GPU server fails permanently, or MSCS coursework requires datacenter-tier compute the 4070 Ti Super can't provide, migrate affected workloads to Vultr fractional A40, RunPod, Modal, etc. QMD's `RemoteLLM` is OpenAI-compatible at the wire, so this is a config change, not a refactor.
...
The Ollama inference layer holds no state worth backing up — models are pulled from HuggingFace via Ollama's registry, and the cache regenerates on demand. The recoverable artifacts on the GPU server are: Ubuntu Server version + driver state, Modelfiles for imported GGUFs, the Ollama systemd unit overrides, the Obico ML docker-compose config, and the QMD fork's git history. Keep all of these in version control / config-as-code so a clean rebuild on replacement hardware is straightforward. The GPU server is a physical single point of failure — if it dies, search degrades to BM25 and Obico stops detecting print failures until restoration. Acceptable given the graceful-fallback design for Lattice; Obico-side risk is bounded by individual print duration.
...
Mild exception for the Ollama endpoint: it's worth running a local Ollama instance during dev work that touches the QMD config plumbing, so changes can be exercised without having to be on the home network. Not a staging environment — just a faster feedback loop. The dev Ollama can live on a laptop or the dev machine; it doesn't need GPU acceleration to validate that config plumbing is correct.
...
Spine refuses non-HTTPS requests. Caddy handles TLS; the spine fails closed if it ever sees a non-HTTPS request. Spine → GPU server is over Tailscale (encrypted by WireGuard at the network layer, not application HTTPS) — that's an acceptable trade because the traffic never leaves the tailnet. If the inference layer ever moves to a public endpoint, that link gets HTTPS too.

### The inference layer: where does each workload run?

A recurring design question once Phase 6 is in place. The default answer: **use whatever the rest of the inference layer uses unless there's a specific reason not to**. Concretely, this means the GPU server's Ollama instance by default; deviate only when:
...
### Inference layer review triggers

The home GPU server has near-zero ongoing cost (~$5–10/month electricity, mostly under load). The relevant question isn't "is the bill getting too high" — it's "is this still the right deployment target?" The following triggers prompt a review independent of timeline:

- **Hot-path latency consistently above 2 seconds.** Something's wrong — the 4070 Ti Super should handle this workload comfortably. Investigate before migrating.
- **Daily uptime falls below ~95%** (home network outages, power issues, hardware instability). The graceful BM25 fallback handles individual outages, but a chronic reliability issue undermines the system. Consider UPS, networking improvements, or migrating the inference layer to cloud.
- **GPU server hardware fails permanently.** Migration becomes urgent; lexical-only fallback bridges the gap while a replacement is sourced.
- **MSCS coursework demands datacenter-tier GPU compute** the 4070 Ti Super can't provide (multi-GPU training, very large model fine-tuning, etc.). Route those specific workloads to cloud or campus resources without displacing the Lattice inference layer — they're additive, not replacement. *(This is the kept-but-narrowed version of the original "revisit when MSCS lands" trigger.)*
- **A single Phase 9+ workload doesn't behave well** alongside QMD or Obico (memory contention, latency interference, etc.). Move that specific workload to cloud or a separate Ollama instance rather than re-architecting the whole server.

For each of these, the migration target is determined by what's available at the time:

- **Vultr fractional A40** (4GB slice ~$60/mo, 8GB ~$120/mo, 12GB ~$210/mo at always-on): drop-in via env-var change. Best for "I just need it to work, fast."
- **RunPod pod or hosted APIs for individual workloads**: situational; bakeoff first.
- **Replacement home GPU**: if the 4070 Ti Super fails and is replaceable in-kind, that's the cheapest restoration.
...
The current architecture has no cloud inference dependencies — the GPU server is local hardware on Tailscale. Phase 9+ may introduce hosted-LLM APIs (Claude for summarization, etc.) for specific workloads. New operational concerns to apply at that point:

- **Cost visibility**: when hosted APIs are added, set per-provider billing alerts. Looking at bills once a month isn't enough feedback when adding multiple providers.
- **Vendor portability**: QMD's `RemoteLLM` is OpenAI-compatible at the wire. Migrating between providers is a URL change. Don't tie hard to provider-specific features when standard alternatives exist.
- **Outage resilience**: QMD's `HybridLLM` already degrades gracefully to lexical-only when remote inference is unreachable. Verify quarterly that the fallback works — power off the GPU server for a minute and confirm search still returns BM25 results with the degraded indicator.
...
The thing to most avoid: over-building any phase before living with the previous one. The friction surfaces faster than design can predict. Phase 6 is itself proof of this several times over — first it was a Phase 9+ deferred item until usage forced it forward; then its scope shrank by an order of magnitude when prior art (PR #629) turned out to already exist; then its deployment target shifted from "cloud serverless" to "home Orin Nano" once cost reality and outside context (MSCS plans, robotics priorities) became clear; then shifted again from Orin to a dedicated GPU server once a free RTX 4070 Ti Super made consolidation viable. Each of these was a course-correction made cheap by not committing to the previous framing too hard.
