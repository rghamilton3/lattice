...
| `SIGNAL_NOTIFICATION_POSTURE`        | `standard`                    | Controls Signal relay text replies: `quiet` (none), `standard` (failures + classifier-routed), `active` (all including unclassified)            |
| `TRANSCRIPTION_NOTIFICATION_POSTURE` | `standard`                    | Controls voice-transcription attention events: `quiet` (none), `standard` (completions + terminal failures), `active` (all including transient) |
| `LATTICE_ASR_MODEL`                  | read from config.toml         | Overrides `[spine.qmd] asr_model`; enables voice-note transcription via the /v1 ASR shim                                                        |
